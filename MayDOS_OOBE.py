...
if not os.path.isfile('important/Version.ver'):
    with open('important/Version.ver', 'w', encoding='gbk') as f:
        f.write(f"{CODE}\n")
        f.close()
if not os.path.isfile('important/download/cg.txt'):
    path_url = os.getcwd() + "\\"
...
while True:
    ...
    print(
        'Dev:这些更新经过Annie Cathy验证，但相较于前一更新通道，此更新通道可能不稳定。此更新通道更新频率为每月一次。适合Windows爱好者使用。')
    print(
        'Canary:此更新不由Annie Cathy验证，且相较于前两个更新通道，此更新通道极不稳定。此更新通道更新频率为每周一次。适合高度技术性用户使用。')
